...
object NaiveBayes {
  ...
  */
  ...
  def main(args: Array[String]) = {
    ...
    println("Processing time; %d milliseconds".format((t1 - t0) / 1e6))
    val model = train(mat)
    val t2 = System.nanoTime()
    println("Training time; %d milliseconds".format((t2 - t1) / 1e6))
    val outcome = validate(mat, n_folds, model)
    val t3 = System.nanoTime()
    println("Validation time; %d milliseconds".format((t3 - t2) / 1e6))
